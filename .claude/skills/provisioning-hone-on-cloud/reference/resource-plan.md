...
# 7. Env vars (loop; --action set upserts one key, preserves others). Do NOT set HONE_DB_* — the `hone`
#    telemetry connection defaults to the app's own database, which Cloud configures from the attached
#    Postgres. Set HONE_DB_* only to isolate telemetry onto a SEPARATE database.
#    Leave HONE_QUEUE_CONNECTION UNSET → job uses the default = managed queue.
for kv in \
  'APP_URL=https://<env-url>' ; do
  k=${kv%%=*}; v=${kv#*=}
