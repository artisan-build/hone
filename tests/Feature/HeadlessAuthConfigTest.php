...
it('merges package defaults and owns the human auth foundation through the released provider', function (): void {
    ...
    expect(data_get($rootComposer, 'require.artisan-build/built-for-cloud'))->toBe('^0.12.1')
        ->and(data_get($serverComposer, 'require.artisan-build/built-for-cloud'))->toBe('^0.12.1')
        ->and(InstalledVersions::getPrettyVersion('artisan-build/built-for-cloud'))->toBe('v0.12.1')
